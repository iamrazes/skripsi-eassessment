...
<body class="flex flex-col bg-[#F5F7F8] text-textColor antialiased">
    <nav
    ...
    <main class="flex flex-col lg:flex-row gap-x-8 -mt-14 lg:px-16 mb-8 px-4">
        <div class="flex flex-col gap-y-8 flex-grow">
            <div class="flex flex-col gap-y-4 bg-white shadow-button rounded-lg p-6">
                ...
                <div class="flex flex-col bg-slate-100 min-h-96 rounded-lg p-4 items-center overflow-hidden">
                    <p class="whitespace-pre-line w-full lg:text-lg ">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum non enim in lorem facilisis finibus ut in nisl. Nulla facilisi. Phasellus placerat justo ac feugiat congue. Donec in laoreet dolor, vel pulvinar nulla. Morbi vel lectus eros. Nunc consequat interdum odio, sit amet laoreet velit condimentum ac. Fusce sodales mi in dui ultrices lobortis. Nulla in varius leo, non commodo quam. Vivamus et tristique est. Aenean eget tempus eros, nec imperdiet augue. Proin luctus, velit congue facilisis elementum, lectus libero maximus turpis, sit amet suscipit augue est vel nulla. Nulla laoreet nulla a luctus placerat. Nulla tincidunt eros vitae libero volutpat viverra. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum non enim in lorem facilisis finibus ut in nisl. Nulla facilisi. Phasellus placerat justo ac feugiat congue. Donec in laoreet dolor, vel pulvinar nulla. Morbi vel lectus eros.
                    </p>
            ...
            <div class="flex flex-col gap-y-8 text-">
                ...
                <div class="flex justify-start shadow-button">
                    <a href="/src/assessment.html" class="bg-white text-center rounded-l-xl px-6 py-4 flex-grow">Previous</a>
                    <a href="/src/cbt-2.html" class="bg-white text-center rounded-r-xl border-l-2 px-6 py-4 flex-grow">Save & Next</a>
                </div>
            </div>
        </div>
        <div class="flex flex-col bg-white shadow-button rounded-lg min-h-screen flex-grow lg:max-w-[400px] mt-8 lg:mt-0 shrink-0 ">
            ...
            <div class="flex flex-col gap-y-2 place-content-end border-t py-4 align-middle">
                <span class="flex gap-x-2 px-4"><div class="w-5 h-5 rounded-md bg-white border shadow"></div>Not Answered</span>
                <span class="flex gap-x-2 px-4"><div class="w-5 h-5 rounded-md bg-accent-1 shadow"></div>Answered</span>
                <span class="flex gap-x-2 px-4"><div class="w-5 h-5 rounded-md bg-white border-accent-1 border-2 shadow"></div>Currently Seeing</span>
                <span class="flex gap-x-2 px-4"><div class="w-5 h-5 rounded-md bg-orange-300 shadow"></div>Marked</span>
                <span class="flex gap-x-2 px-4"><div class="w-5 h-5 rounded-md bg-accent-2 shadow"></div>Marked For Review</span>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        document.querySelectorAll('.choice-button input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('click', function () {
                if (this.checked) {
                    document.querySelectorAll('.choice-button input[type="checkbox"]').forEach(otherCheckbox => {
                        if (otherCheckbox !== this) {
                            otherCheckbox.checked = false;
                            otherCheckbox.nextElementSibling.style.backgroundColor = ''; // Reset other checkboxes' styles
                        }
                    });

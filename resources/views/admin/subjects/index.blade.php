...
@section('content')
    ...
    <div class="flex bg-white flex-col rounded-lg shadow-button mt-8">
        <h1 class="font-semibold px-6 pt-4 text-lg">Subjects</h1>

        <div class="px-6 rounded-lg pt-4">
            <label for="name" class="block font-semibold text-gray-700">Subject Name</label>
            <form action="{{ route('admin.subjects.store') }}" method="POST" class="flex items-center gap-4">
                @csrf
                <div class="flex-1">
                ...
                <div class="flex-shrink-0">
                    <button type="submit" class="bg-blue-500 text-white px-4 py-2 mt-1 rounded-md h-10">Create Subject</button>
                </div>
            </form>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 border-b border-gray-200" id="resultsTable">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($subjects as $index => $subject)
                        <tr>
                            <td class="pl-6 py-4 numbering-cell">{{ $index + 1 }}.</td>

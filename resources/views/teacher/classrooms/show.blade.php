...
@section('content')
    <div class="mt-4 lg:mt-8 mx-4 lg:mx-0 bg-white shadow-button rounded-lg pb-8 pt-4">
            ...
            <table class="min-w-full divide-y divide-gray-200 border-b border-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($classroom->students as $index => $student)
                        <tr>
                            <td class="lg:px-6 px-3 py-4 w-4">{{ $index + 1 }}.</td>
                            <td class="lg:px-6 px-3 py-4">{{ $student->user->name }}</td>
                            <td class="px-6 py-4 text-end">{{ $student->student_id }}</td>

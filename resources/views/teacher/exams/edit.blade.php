...
@section('title')
    <title>Edit Exam - {{ config('app.name') }}</title>
@endsection
        ...
        <div class="my-4">
            <form action="{{ route('teacher.exams.update', $exam->id) }}" method="POST">
                ...
                <div class="flex flex-col my-3">
                    ...
                    <textarea name="description" id="description" class="rounded-lg border-gray-400 mt-1 h-40">{{ old('description', $exam->description) }}</textarea>
                </div>
                <button type="submit"
                    class="bg-accent-1 hover:bg-gradient-to-r from-accent-1 to-accent-2 text-white py-2 px-4 flex w-full text-center justify-center font-semibold rounded-lg text-lg mt-4">Update
                    Exam</button>
            </form>
        </div>
    </div>
